...
DATOS SEGMENT
	COUNTER DB ?   ; We write "DB" because we want a single byte variable. ? indicates "non-initialized"
	GRAB DW 0CAFEh ; The 0 is needed to succeed on compilation
	TABLE100 DB 100 dup (?)  ; We want 100 bytes, so we write db 100 dup. 
							 ; We just want to reserve memory, so we don't initialize them
...
INICIO PROC
	...
	; SECOND INSTRUCTION
	MOV AX, GRAB            		; GRAB is a word long, needs the whole register AX
	...
	MOV COUNTER, AH 		; AX contains GRAB from the last instruction.
							; The most significative byte is in the higher part (AH) due to the endianess of 8086 processor (little-endian).
